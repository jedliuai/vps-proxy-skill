...
在脱敏 worklog 中记录：目标别名/区域（不写秘密）、Git revision、版本、协议与端口、直连/Cloudflare 模式、每层验收结果、未完成项、证书续期、快照/备份位置、下一步和预算告警是否真的创建。每个任务阶段的“已配置”和“已验证”分开。

出现工具/权限/网络阻塞时，完成安全检查后报告缺的具体条件，不循环重新部署。新会话从记录和当前只读状态续接，不重复轮换或创建 Worker/KV。

原实例 2026-08-31 Trojan 100/100（约151秒）、HY2和REALITY各6/6 是历史案例，不是本次测试结果、吞吐量或 SLA。新用户实例在未测前一律记为未验收。
