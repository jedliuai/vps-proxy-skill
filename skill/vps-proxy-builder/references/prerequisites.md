...
记录非敏感项：目标 VPS 公网 IPv4、提供商/区域、系统/CPU、已有业务、SSH 端口/用户名/本地私钥路径、用户客户端、可控制域名、Cloudflare 是否可选、预算/流量上限、恢复控制台。不要索要私钥正文。

本 bootstrap 固定支持 Debian 13 x86_64 + systemd + SSH 22，建议专用主机。历史运行于美国 GCP e2-micro（1 GB 内存、2 GB swap），不是最低规格保证或当前购买建议。不自动重装用户系统。ARM、只有 NAT 端口映射、只有 IPv6、其他发行版先停止自动安装并解释适配成本。
