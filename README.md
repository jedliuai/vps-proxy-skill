...
**小白也能一步开始，不需要学习繁杂的 VPS、网络或 VPN 搭建知识。**

<a id="quick-start"></a>

## 🚀 只需把这句话发给 Codex

你只需要准备自己的 VPS 和域名，把下面的话发给 Codex。它会询问连接信息，协助连好 SSH；检查并授权后，后续全程由它全自动执行。
...
**Hysteria2、Trojan、VLESS REALITY**——本项目选用的当前最强的三种互补方案，Codex 会完成配置和检查，你完全不需要研究该怎么搭。
...
这里的 VPN、翻墙、科学上网或“魔法”指私人代理节点，不提供免费服务器或公共订阅。购买、账号登录、必要授权和最终试用由你完成，服务器与域名等费用自理。自动部署以条件满足为前提；不要公开订阅链接或密钥。
